...
(function() {
  ...
  var sha256 = function a(b) {
      ...
      for (var d, e, f = Math.pow,
      g = f(2, 32), h = "length", i = "", j = [], k = 8 * b[h], l = a.h = a.h || [], m = a.k = a.k || [], n = m[h], o = {},
      p = 2; 64 > n; p++) if (!o[p]) {
  ...
  skipButton.style.textShadow = ` - 1px - 1px 0#000,
  1px - 1px 0#000,
  -1px 1px 0#000,
  1px 1px 0#000`;
  ...
  setInterval(() => {
      if ((window.location.href !== previousUrl) && !window.location.href.includes('#searching') && !window.location.href.includes('#bottom-sheet')) {
          ...
          if (document.querySelector('div[role="slider"]')) {
              document.querySelector('div[role="slider"]').style.background = "none";
              document.querySelector('div[role="slider"]').parentNode.style.background = "none";
          }
          if (window.location.href.includes('watch?v=')) {
              gradientStops = [];
              gradientStophighlight = [];
              skipSegments = [];
              index3 = false;
          ...
          if (document.querySelector('video').currentTime > 0 && document.querySelector('div[role="slider"]') && window.location.href.includes('watch?v=') && !index3) {
              ...
              (async() => {
                  try {
                      ...
                      if (!response.ok) {
                          throw new Error(`HTTP error ! Sponsorblock server down ! Status: $ {
                              response.status
                          }`);
                      }
                      ...
                      for (const video of data) {
                          ...
                          segments)`;
                          const cat_n = video.segments.map(e => e.category).sort().reduce((acc, e) => (acc[e] = (acc[e] || 0) + 1, acc), {});
                          ...
                          for (const segment of video.segments) {
                              ...
                              if (segment.category === "sponsor") {
                                  gradientStops.push(`transparent $ {
                                      startPosition + 0.5
                                  } % , green $ {
                                      startPosition + 0.5
                                  } % , green $ {
                                  ...
                                  gradientStophighlight.push(`transparent $ { ((start) / (video_obj.duration)) * 100
                                  } % , red $ { ((start) / (video_obj.duration)) * 100
                                  } % , red $ { ((start) / (video_obj.duration)) * 100 + 1
                                  } % , transparent $ { ((start) / (video_obj.duration)) * 100 + 1
                                  } % , transparent $ { ((start) / (video_obj.duration)) * 100 + 1
                                  } % `);
                          ...
                          intervalId = setInterval(() => {
                              if (document.querySelector('div[role="slider"]')) {
                                  document.querySelector('div[role="slider"]').style.backgroundSize = `100 % 25 % `;
                                  document.querySelector('div[role="slider"]').style.backgroundPosition = `center center`;
                                  document.querySelector('div[role="slider"]').style.backgroundRepeat = 'no-repeat';
                                  document.querySelector('div[role="slider"]').style.backgroundImage = `linear - gradient(to right, $ {
                                      gradientStops.join(', ')
                                  })`;
                                  document.querySelector('div[role="slider"]').parentNode.style.backgroundPosition = `center center`;
                                  document.querySelector('div[role="slider"]').parentNode.style.backgroundRepeat = 'no-repeat';
                                  document.querySelector('div[role="slider"]').parentNode.style.backgroundSize = `100 % 25 % `;
                                  document.querySelector('div[role="slider"]').parentNode.style.backgroundImage = `linear - gradient(to right, $ {
                                      gradientStophighlight
                                  })`;
                                  ...
                                  document.querySelector('div[role="slider"]') ?.addEventListener('mousemove',
                                  function(event) {
                                      highlightSpan.style.display = "block";
                                  });
                                  document.querySelector('div[role="slider"]') ?.parentNode.addEventListener('mouseout',
                                  function(event) {
                                      highlightSpan.style.display = "none";
                                  });
                              }
